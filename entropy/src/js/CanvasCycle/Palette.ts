...
export class Cycle {
	...
	low: number = 0
	high: number = 0
	cycleAmount: number = 0
	
	constructor(r: number, rev: number, l: number, h: number) {
		this.rate = r;
		this.reverse = rev;
		this.low = l;
		this.high = h;
...
export class Palette {
	...
	static PRECISION: number = 100
	static USE_BLEND_SHIFT: number = 1
	static CYCLE_SPEED:number = 280
	static ENABLE_CYCLING:number = 1
		
	// this utility function allows for variable precision floating point modulus
	static DFLOAT_MOD(a: number, b: number) { 
		return (Math.floor(a*Palette.PRECISION) % Math.floor(b*Palette.PRECISION))/Palette.PRECISION
	}

	
	constructor (clrs: Array<Color>, cycls: Array<Cycle>) {
		// class constructor
		this.colors = [];
		this.baseColors = [];
	...
	copyColors(source: Array<Color>, dest: Array<Color>) {
	...
	reverseColors(colors: Array<Color>, range: Cycle) {
	...
	fadeColor(sourceColor: Color, destColor: Color, frame: number, max: number) {
	...
	shiftColors(colors: Array<Color>, range: Cycle, amount: number) {
	...
	blendShiftColors(colors: Array<Color>, range: Cycle, amount: number) {
	...
	cycle(sourceColors: Array<Color>, timeNow: number, speedAdjust: number, blendShift: boolean) {
	...
	fade(destPalette: Palette, frame: number, max: number) {
	...
	fadeToColor(color: Color, frame: number, max: number) {
	...
	burnOut(frame: number, max: number) {
