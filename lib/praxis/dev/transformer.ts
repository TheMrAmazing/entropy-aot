...
export default function (program: ts.Program) {
    ...
    return (context: ts.TransformationContext) => {
      return (sourceFile: ts.SourceFile) => {
		...
		const asNamedImports:Map<string, [string, string]> = new Map()
		const namespaceImports:Map<string, string> = new Map()
		const defaultImports:Map<string, string> = new Map()
		let blockScopeCancels: string[][] = []
		...
		const visitor = (node) => {
			...
			if(ts.isIdentifier(node)) {
				let identifier = node.escapedText as unknown as string
				if(blockScopeCancels.flat().includes(identifier)) {
					...
					if (defaultImports.get(identifier)) {
						res = ts.factory.createCallExpression(
					...
					if(namespaceImports.get(identifier)) {
						res = ts.factory.createCallExpression(
							ts.factory.createIdentifier('require'),
							undefined,
							[ts.factory.createStringLiteral(namespaceImports.get(identifier))]
						)
						return res	
					}
					...
					if (asNamedImports.get(identifier)) {
						res = ts.factory.createPropertyAccessExpression(
							ts.factory.createCallExpression(
								ts.factory.createIdentifier('require'),
								undefined,
								[ts.factory.createStringLiteral(asNamedImports.get(identifier)[1])]
							), 
							ts.factory.createIdentifier(asNamedImports.get(identifier)[0])
						)
						return res
					}
				}
