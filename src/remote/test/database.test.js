import {fnArg} from '../Controller.js'
...
test('set object', async() => {
	...
	check(await globalThis.db().users[0].roles[1], {
		foo: 'bar',
		baz: 42,
		loopy: {$ref: '#users/0/roles/1/loopy'}
	})
})
...
test('apply function arg', async() => {
	...
	check(x, {
		name: 'role3',
		innerBoolArray: {'$ref': '#users/0/roles/2/innerBoolArray'},
		innerUndefinedArray: {'$ref': '#users/0/roles/2/innerUndefinedArray'},
		innerUndefinedProp: undefined
	})
})
